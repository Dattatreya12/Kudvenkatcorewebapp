...
namespace Kudvenkatcorewebapp.Controllers
{
    ...
    public class StockAdminController : Controller
    {
        ...
        [HttpGet]
        public async Task<IActionResult> AddDailyIncomeLoss()
        {
            ViewBag.tradelist = new SelectList(await _brokerRepository.GetAllstockListusingIenumerable(), "Id", "Stockname");
            return PartialView("_showProfitLossView");
        }

        [HttpPost]
        public JsonResult AddDailyIncomeLoss(AddProfitLoss addProfitLoss)
        {
            if (addProfitLoss.Option == "" || addProfitLoss.IncomeLoss == 0)
            {
                return new JsonResult(new { success = false, message = "Fill the all fields" });
            }
            int i = _brokerRepository.AddProfitandLoss(addProfitLoss, "InsertProfitLoss");
            if (i > 0 || i < 0)
            {
                int sngid = 1;
